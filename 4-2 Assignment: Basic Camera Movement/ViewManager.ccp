...
void ViewManager::ProcessKeyboardEvents()
{
    ...
    {
        g_pCamera->ProcessKeyboard(UP, gDeltaTime * cameraSpeed);  // Q moves up
    }
    if (glfwGetKey(m_pWindow, GLFW_KEY_E) == GLFW_PRESS)
    {
        g_pCamera->ProcessKeyboard(DOWN, gDeltaTime * cameraSpeed);  // E moves down
    }

    // Increase or decrease camera speed using + and -
    if (glfwGetKey(m_pWindow, GLFW_KEY_KP_ADD) == GLFW_PRESS || glfwGetKey(m_pWindow, GLFW_KEY_EQUAL) == GLFW_PRESS)
    {
        cameraSpeed += 2.0f;  // Increase speed more noticeably
    ...
    {
        cameraSpeed = std::max(0.5f, cameraSpeed - 2.0f);  // Decrease speed, but don't let it go below 0.5
